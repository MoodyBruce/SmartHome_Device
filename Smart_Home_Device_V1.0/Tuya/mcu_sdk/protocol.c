...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_LIGHT_VALUE, DP_TYPE_VALUE},
  {DPID_MINLIGHT_SET, DP_TYPE_VALUE},
  {DPID_LIGHT_ALARM, DP_TYPE_ENUM},
  {DPID_LIGHT_SWITCH, DP_TYPE_BOOL},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_MINLIGHT_SET,temp_min); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_LIGHT_ALARM,light_alarm); //枚举型数据上报;
    mcu_dp_bool_update(DPID_LIGHT_SWITCH,0); //BOOL型数据上报;
...
static unsigned char dp_download_switch_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(switch_1 == 0) {
     device_state = 0x01;    //关设备   //开关关
    }else {
     device_state = 0x02;    //开设备   //开关开
    }
...
/*****************************************************************************
函数名称 : dp_download_minlight_set_handle
功能描述 : 针对DPID_MINLIGHT_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_minlight_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long minlight_set;
    
    minlight_set = mcu_get_dp_download_value(value,length);
    //VALUE类型数据处理
    light_min = minlight_set;
    
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_MINLIGHT_SET,minlight_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_light_switch_handle
功能描述 : 针对DPID_LIGHT_SWITCH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_light_switch_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    light_switch = mcu_get_dp_download_bool(value,length);
    if(light_switch == 0) {
    USB_OFF();    //开关关
    }else {
    USB_ON();    //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LIGHT_SWITCH,light_switch);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_MINLIGHT_SET:
            //亮度下限设置处理函数
            ret = dp_download_minlight_set_handle(value,length);
        break;
        case DPID_LIGHT_SWITCH:
            //灯控开关处理函数
            ret = dp_download_light_switch_handle(value,length);
        break;
